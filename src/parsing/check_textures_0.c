...
static char	*prepare_texture_string(char *line, int j)
{
	char	*sub;
	int		len;
	int		i;
...
static void	assign_texture(char c, char *sub, t_textures *textures)
{
...
void	set_texture(char c, int j, char *line, t_textures *textures)
{
	...
	sub = prepare_texture_string(line, j);
...
int	check_line(t_cub3d *cub3d, char *line, t_textures *textures)
{
	...
	if (is_letter(line[j]))
	{
		...
		skip_spaces(line, &j);
		process_texture_found(cub3d, line, j, z);
	}
